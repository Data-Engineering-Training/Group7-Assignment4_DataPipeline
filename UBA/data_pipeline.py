...
def ingest_accounts(cursor):
    try:
        with open('data/accounts.csv', 'r') as csvfile:
            csvreader = csv.DictReader(csvfile)
            for row in csvreader:
                cursor.execute(
...
def ingest_transactions(cursor):
    try:
        with open('data/transactions.csv', 'r') as csvfile:
            csvreader = csv.DictReader(csvfile)
            for row in csvreader:
                cursor.execute(
...
if __name__ == "__main__":
    try:
        # Connect to MySQL
        cnx = mysql.connector.connect(user='root', password='root', host='localhost', database='UBA_database')
        cursor = cnx.cursor()
